...
jest.mock('../IssueProvider', jest.fn().mockImplementation(() => ({
   __esModule: true, //When module has multiple exports
   default: jest.fn().mockImplementation(() => ({
...
jest.mock('../StateProvider', jest.fn().mockImplementation(() => ({
   __esModule: true, //When module has multiple exports
   default: jest.fn().mockImplementation(() => ({
      lastUidSynced: {
      ...
      lastSynced: {
         get: jest.fn(() => Promise.resolve(new Date("2022-02-21").toISOString())),
         set: jest.fn(() => Promise.resolve())
      }
   })),
})))
...
describe('Mail2Issue', () => {
   ...
   it('should get create 3 issues', async () => {
      const mailProvider = new MailProvider({ emailAddress: 'abc@efg.com', password: '1234', host: 'mail.server.com' });
      ...
      expect(issueProvider.createIssue).toHaveBeenCalledWith(createIssuesFixture[0]);
      expect(issueProvider.createIssue).toHaveBeenCalledWith(createIssuesFixture[1]);
      expect(issueProvider.createIssue).toHaveBeenCalledWith(createIssuesFixture[2]);
   ...
   it('should handel replays to  comments ', async () => {
      ...
      const mailProvider = new MailProvider({ emailAddress: 'abc@efg.com', password: '1234', host: 'mail.server.com' });
      const newFixture = {...fixture[0], subject: 'Re:[:10001]abc'} as unknown as FetchedEmail;
      jest.spyOn(mailProvider, 'fetchEmailsByUID').mockImplementation(() =>  Promise.resolve([newFixture]));
      ...
      expect(issueProvider.commentIssue).toHaveBeenCalledWith(10001, newFixture.body);
